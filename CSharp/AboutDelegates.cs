...
namespace DotNetKoans.CSharp
{
	public class AboutDelegates : Koan
	{
		...
		public void DelegatesAreReferenceTypes()
		{
			...
			BinaryOp op;
			//Assert.Null(op); //comment it out, or just fill in with null b/c code won't build with the correct answer (pictured here)
		}
		[Koan(2)]
		...
		{
            Assert.Equal(6, passed(3,3)); //how do we know this is math.add and not math.subtract?? see where the method is called on line 74!!
        }
		[Koan(6)]
		...
		public void DelegatesAreImmutable()
		{
			...
			//Assert.Same(a, original);  //There was nothing to fill in, but had to comment this line out to pass to next test
		}
		...
		public void RemovingMethods()
		{
			...
			adding += f.Add10;
			Assert.Equal(2, adding.GetInvocationList().Length);
            adding -= f.Add5; //Remove Add5 from the invocation list
			Assert.Equal(1, adding.GetInvocationList().Length);
		...
		{
			Assert.Equal(42, x); //how come this passes even though theres  no value for x?? See line 159!
		}
		...
		[Koan(15)]
		public void ActionInTheBcl() //Um, what the BCL???
		{
