...
namespace DotNetKoans.CSharp
{
    public class AboutArrays : Koan
    {
        ...
        public void PushingAndPopping()
        {
            ...
            stack.Push("last");
            Assert.Equal(new object[] { (string)"last", (int)2, (int)1 }, stack.ToArray()); //needs to be FIXED
        ...
        public void Shifting()
        {
            ...
            list.AddFirst("Say");
            Assert.Equal(new string[] { (string)"Say", (string)"Hello", (string)"World" }, list.ToArray());

            list.RemoveLast();
            Assert.Equal(new string[] { (string)"Say", (string)"Hello" }, list.ToArray());

            list.RemoveFirst();
            Assert.Equal(new string[] { (string)"Hello" }, list.ToArray());

            list.AddAfter(list.Find("Hello"), "World");
            Assert.Equal(new string[] { (string)"Hello", (string)"World" }, list.ToArray());
        }
