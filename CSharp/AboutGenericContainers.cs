...
namespace DotNetKoans.CSharp
{
	public class AboutGenericContainers : Koan
	{
		...
		public void MustCastWhenRetrieving()
		{
			...
			list.Add(42);
			int x = 0;
			x = (int)list[0]; //I just un-commented this line to make it pass
			Assert.Equal(x, 42);
		}
		[Koan(4)]
		...
		public void Boxing()
		{
			short s = 5;
			object os = s;
			Assert.Equal(s.GetType(), os.GetType()); //WTF?
			Assert.Equal(s, os);                    //WTF??
		...
		public void RemovingItems()
		{
			...
			list.Remove(2); //  we removed the number 2, not the item at index 2
			Assert.Equal(new int[] { 1, 2, 3 }, list.ToArray()); 
		}
		[Koan(15)]
